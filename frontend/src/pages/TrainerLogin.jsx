...
import { UserCheck, Lock, Mail, ArrowLeft, Loader } from 'lucide-react';
...
const TrainerLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`${import.meta.env.VITE_API_URL}/auth/trainer/login`, {
        email,
        password
      });

      const { token, user } = response.data;
      loginTrainer(token, user);
    ...
    <div className="min-h-screen flex items-center justify-center bg-background p-6 relative">
      ...
      <div className="w-full max-w-md bg-surface p-10 shadow-2xl border border-gray-800 rounded-xl relative overflow-hidden">
        ...
        <div className="text-center mb-10 relative z-10">
          ...
          <h2 className="text-3xl font-extrabold text-white tracking-tight mb-1">COACH PORTAL</h2>
          <p className="text-xs font-bold text-textSecondary tracking-widest uppercase">Elite Trainers Login</p>
        </div>
